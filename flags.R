...
no_land_degradation   <- 0                  # if 1, no land-degradation whatsoever. (To investigate macro engine)
flag_agri_init        <- 2                  # if 1, 50% sustainable, if 2 25% susustainable, if 3 15% sustainable,  if 0 no sustainables. 
...
flag_deforestation    <- 1                  # if 1, deforestation allowed for conventional famers
...
flag_suspend_ergodic  <- 1                  # if 1, deforestation, land abandonment and auction are suspended during transient (lenght set in parameters). Set to 0 if deforestation and land abandonment are off! Note, switch and soil degradation are always suspended. 
...
random_initialization_flag <- 0             # if not 0 (baseline) randomize all initialization across mc (sustainable positions, forests positions, vintages, productivities)