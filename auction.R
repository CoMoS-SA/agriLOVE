auction_f <- function(j){
    ...
    for (zz in 1:dim(existing_producers)[1]) {
        property    <<- which(world[,,t-1,p]==zz, arr.ind = T)                                                                          # t-1 as if the current zz just bought another property from z, then close_cell funtion doesn't work
        value_sale  <<- min_land_price
        if(existing_producers[zz,t,p]==1 & (z!= zz) & ((wealth_share_bid*p_wealth[zz,t,p]) > value_sale) ){                             # if z producer is still active and has enough money to bid; excluded owner of cell on sale                                                                                                 # find the closest cell among those owned by producer z
            close_cell      <<- ray_closest_cell(property,cells_on_sale[j,"row"],cells_on_sale[j,"col"])                            # note, close cell is an array, so not use matrix subsetting notation in the following
    ...
    bidders         <<- which(p_bidding==TRUE)                                                                                          # positiom of those placing a bid
    ...
    if(length(bidders)>0){ 
        ...
        mkt_share[bidders[ordered_bids[1]],t,p]                     <<- mkt_share[bidders[ordered_bids[1]],t,p] + mkt_share_cell_on_sale[j, "ms"]   #buyers gets old cell mktshares
