...
#### Analysis settings
experiment_naming<- 2                                           # set to 1 to retriece duynamic names for experiments, se to 2 for two levels experiemnts, otherewise use "last_run.RData".
...
report           <- 0                                           # do you want automatic analysis to be permormed automatically at the end of the run? If yes, set to 1
...
mc   <- 500                                                      # number of montecarlo replications
...
##### Initialization
if(scen_flag==0){
    if(random_initialization_flag==0){
        source("initialization.R")
    }else{
        source("initialization_random.R")
    }
}else{
